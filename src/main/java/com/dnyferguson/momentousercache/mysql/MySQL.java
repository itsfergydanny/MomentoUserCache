...
public class MySQL {
    ...
    public MySQL(MomentoUserCache plugin) {
        this.plugin = plugin;
    ...
    private void createTables(String db) {
        executeStatementSync("CREATE TABLE IF NOT EXISTS `" + db + "`.`users` ( `uuid` VARCHAR(36) NOT NULL , `ign` VARCHAR(16) NOT NULL, `head_texture` VARCHAR(2048) NOT NULL, `last_updated` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP , UNIQUE (`uuid`)) ENGINE = InnoDB;");
    }
